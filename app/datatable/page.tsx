...
export default function DataTable() {
  ...
  const fetchData = async () => {
    ...
    try {
      const bcmrAPI = await fetch(bcmrServer + tokenCategory);
      ...
      if (
        ...
        let tokenTotalReservedAmount =
          tokenReservedSupplyData.data.transaction?.[0]?.authchains?.[0]
            ?.authhead?.identity_output?.[0]?.fungible_token_amount;

        tokenTotalReservedAmount = BigInt(tokenTotalReservedAmount);

        // Convert to a decimal form
        tokenTotalReservedAmount =
          tokenTotalReservedAmount / BigInt(Math.pow(10, tokenDecimals));
      ...
      let totalValueLocked =
        satoshisToBCH(cauldronLiquidityData.result.bch) * bchPrice.toFixed(2);
  ...
  // /table has formatting for Tremor

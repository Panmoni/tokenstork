...
//
// Streaming model (2026-06-19): All 20 queries fire in one Promise.all
// (same parallelism as original). The token shell (name, icon, type,
// description) renders immediately from the already-fetched token query.
// Everything else streams in via data.streamed — SvelteKit renders the
// header instantly and fills in price/chart/holders/etc. as they resolve.
...
export const load: PageServerLoad = async ({ params, fetch, url, locals }) => {
	...
	// ── Token query (renders shell immediately) ───────────────────────
	const tokenRes = await query<TokenRow>(
		`SELECT
			t.category, t.token_type, t.genesis_block, t.genesis_time,
			t.first_seen_at, t.genesis_txid, t.authchain_head_txid,
			m.name, m.symbol, m.decimals, m.description, m.icon_uri,
			m.bcmr_publication_uri, m.bcmr_source, m.bcmr_body,
			...
			s.current_supply::text AS current_supply, s.live_utxo_count,
			s.live_nft_count, s.holder_count, s.has_active_minting,
			s.is_fully_burned, s.gini_coefficient, s.verified_at,
	...
	// ── Resolve BCMR metadata + icon status from token row ────────────
	const hasBcmrMetadata =
		row.name != null || row.symbol != null || row.description != null ||
		row.icon_uri != null || bcmr != null;
	const iconStatus = resolveIconStatus({
		iconUri: row.icon_uri, clearedHash: row.icon_cleared_hash,
		moderationState: row.icon_state, blockReason: row.icon_block_reason,
		fetchError: row.icon_fetch_error, hasScanRow: row.icon_scan_present,
		hasBcmrMetadata
	});

	// ── Fire all data queries in one batch (original parallelism) ─────
	// Returns as streamed Promise — token shell renders immediately,
	// price/chart/holders/etc. stream in as they resolve.
	const allData = Promise.all([
		query<HolderRow>(
			`SELECT address, balance::text AS balance, nft_count
			   FROM token_holders WHERE category = $1
			  ORDER BY token_holders.balance DESC, address ASC LIMIT 10`,
			[categoryBytes]
		),
		query<TapswapOfferRow>(
			`SELECT id, has_amount::text AS has_amount, has_commitment,
			        has_capability, has_sats::text AS has_sats,
			        want_sats::text AS want_sats, want_category,
			        want_amount::text AS want_amount, want_commitment,
			        want_capability, maker_pkh, listed_block, listed_at
			   FROM tapswap_offers
			  WHERE has_category = $1 AND status = 'open'
			    AND NOT EXISTS (SELECT 1 FROM token_moderation mod WHERE mod.category = $1)
			  ORDER BY want_sats ASC LIMIT 20`,
			[categoryBytes]
		),
		query<FexRow>(
			`SELECT price_sats, tvl_satoshis::text AS tvl_satoshis
			   FROM token_venue_listings
			  WHERE category = $1 AND venue = 'fex' LIMIT 1`,
			[categoryBytes]
		),
		computeMcapTvlThresholdSats(),
		query<PriceBucketRow>(
			`WITH ordered AS (
			   SELECT ts, price_sats, tvl_satoshis,
			          tvl_satoshis - LAG(tvl_satoshis) OVER (ORDER BY ts) AS tvl_delta
			     FROM token_price_history
			    WHERE category = $1 AND venue = 'cauldron'
			 ...
			 SELECT ${rangeSpec.bucket} AS bucket,
			   ...
			   FROM ordered GROUP BY bucket ORDER BY bucket ASC`,
			[categoryBytes]
		),
		...
		getMovers24h(),
		query<{ venue: 'cauldron' | 'fex'; tvl_satoshis: string | null;
		         pools_count: number | null; pools_total_tvl_sats: string | null;
		         first_listed_at: Date }>(
			`SELECT venue, tvl_satoshis::text AS tvl_satoshis, pools_count,
			        pools_total_tvl_sats::text AS pools_total_tvl_sats, first_listed_at
			   FROM token_venue_listings WHERE category = $1`,
			[categoryBytes]
		),
		query<{ window: '24h' | '7d' | '30d'; price_min: number | null; price_max: number | null }>(
			`WITH windows AS (
			 ...
			 SELECT w.window,
			   ...
			   LEFT JOIN token_price_history h
			     ON h.category = $1 AND h.venue = 'cauldron'
			    AND h.ts > now() - w.interval AND h.price_sats > 0
			  GROUP BY w.window`,
			[categoryBytes]
		),
		query<{ trade_buckets: string; volume_sats: string | null }>(
			`WITH ordered AS (
			   SELECT ts, tvl_satoshis,
			          tvl_satoshis - LAG(tvl_satoshis) OVER (ORDER BY ts) AS tvl_delta
			     FROM token_price_history
			    WHERE category = $1 AND venue = 'cauldron'
			      AND ts > now() - INTERVAL '24 hours'
			 )
			 SELECT COUNT(*) FILTER (WHERE tvl_delta IS NOT NULL AND tvl_delta != 0)::bigint AS trade_buckets,
			        SUM(ABS(tvl_delta)) FILTER (WHERE tvl_delta IS NOT NULL)::text AS volume_sats
			   FROM ordered`,
			[categoryBytes]
		),
		...
		query<{ rank: string }>(
			`WITH self AS (
			   SELECT tvl_satoshis FROM token_venue_listings
			    WHERE category = $1 AND venue = 'cauldron' AND tvl_satoshis IS NOT NULL
			 )
			 SELECT (1 + (SELECT COUNT(*)::bigint
			     FROM token_venue_listings tvl JOIN tokens t ON t.category = tvl.category
			    WHERE tvl.venue = 'cauldron' AND tvl.tvl_satoshis IS NOT NULL
			      AND tvl.tvl_satoshis > self.tvl_satoshis
			      AND NOT EXISTS (SELECT 1 FROM token_moderation mod WHERE mod.category = tvl.category)
			 ))::text AS rank FROM self`,
			[categoryBytes]
		).catch((err) => { console.error('[token detail] TVL rank query failed:', err); return { rows: [] as Array<{ rank: string }> }; }),
		query<{ rank: string }>(
			`WITH self AS (
			   SELECT s.holder_count FROM token_state s
			 ...
			 SELECT (1 + (SELECT COUNT(*)::bigint
			    ...
			    WHERE s.holder_count IS NOT NULL AND s.holder_count > self.holder_count
			      AND NOT EXISTS (SELECT 1 FROM token_moderation mod WHERE mod.category = s.category)
			 ))::text AS rank FROM self`,
			[categoryBytes]
		).catch((err) => { console.error('[token detail] holders rank query failed:', err); return { rows: [] as Array<{ rank: string }> }; }),
		fetchCrc20Detail(categoryBytes).catch((err) => { console.error('[token detail] CRC-20 detail query failed:', err); return null; }),
		query<{ hhi: string | null }>(
			`WITH t AS (SELECT SUM(balance::numeric) AS total, COUNT(*) AS n FROM token_holders WHERE category = $1)
			 SELECT CASE WHEN t.n < 10 THEN NULL WHEN t.total IS NULL OR t.total = 0 THEN NULL
			          ELSE (SELECT (SUM((th.balance::numeric / t.total) ^ 2))::text FROM token_holders th WHERE th.category = $1)
			        END AS hhi FROM t`,
			[categoryBytes]
		).catch((err) => { console.error('[token detail] Herfindahl query failed:', err); return { rows: [] as Array<{ hhi: string | null }> }; }),
	...
	// ── Post-process everything inside .then() → streamed promise ────
	const streamed = allData.then(([
		holdersRes, tapswapRes, fexRes, mcapTvlThresholdSats, priceHistoryRes,
		voteCounts, watchlistCountRes, movers, venueAggregateRes, priceExtremesRes,
		recentTradesRes, reportCountRes, leaderboardStandingsRes, tvlRankRes,
		holdersRankRes, crc20Detail, herfindahlRes, cauldron, cauldronGlobalRes,
		bcmrEligibility, firstNRankResult
	]) => {
		// Fex price
		let fexPriceUSD = 0; let fexTvlUSD = 0;
		...
		// Movers
		const fm = (list: Array<{ categoryHex: string }>, cat: string) => list.findIndex(m => m.categoryHex === cat) + 1;
		const moverRanks = { gainerRank: fm(movers.topGainers24h, category), loserRank: fm(movers.topLosers24h, category), tvlMoverRank: fm(movers.topTvlMovers24h, category) };
		const moverE = movers.topGainers24h.find(m => m.categoryHex === category) ?? movers.topLosers24h.find(m => m.categoryHex === category) ?? null;
		const moverT = movers.topTvlMovers24h.find(m => m.categoryHex === category) ?? null;

		// Holders
		const topHS = (() => { const t = holdersRes.rows[0]; if (!t || !row.current_supply) return null; try { const s = BigInt(row.current_supply); if (s === 0n) return null; return Math.min(100, Number((BigInt(t.balance) * 1_000_000n) / s) / 10_000); } catch { return null; } })();
		const top10HS = (() => { if (!row.current_supply || holdersRes.rows.length === 0) return null; try { const s = BigInt(row.current_supply); if (s === 0n) return null; let sum = 0n; for (const h of holdersRes.rows) sum += BigInt(h.balance); return Math.min(100, Number((sum * 1_000_000n) / s) / 10_000); } catch { return null; } })();

		const watchlistCount = Number(watchlistCountRes.rows[0]?.n ?? 0);

		// Price chart
		const priceBuckets: PriceBucket[] = priceHistoryRes.rows.map(r => ({ ts: Math.floor(r.bucket.getTime() / 1000), priceSats: r.avg_price_sats, volumeSats: r.volume_sats ? Number(r.volume_sats) : null }));
		...
		for (const r of priceExtremesRes.rows) {
			const toU = (px: number | null) => (px != null && px > 0 && bchPriceUSD > 0) ? (px * Math.pow(10, decimals) / 1e8) * bchPriceUSD : null;
			pe[r.window] = { min: toU(r.price_min), max: toU(r.price_max) };
		}

		// Arbitrage
		const cl = vn.cauldron?.tvlSats != null; const fl = vn.fex?.tvlSats != null;
		const tfo = tapswapRes.rows.find(o => o.has_commitment == null && o.has_amount && o.want_amount == null && Number(o.want_sats) > 0);
		const thp = !!tfo; const avp = (cl ? 1 : 0) + (fl ? 1 : 0) + (thp ? 1 : 0);
		let ars: number | null = null;
		if (avp >= 2) {
			const up: number[] = [];
			if (cauldron.priceUSD > 0) up.push(cauldron.priceUSD);
			if (fexPriceUSD > 0) up.push(fexPriceUSD);
			if (thp && tfo?.has_amount) { const as = Number(tfo.want_sats) / Number(tfo.has_amount); if (Number.isFinite(as) && as > 0 && bchPriceUSD > 0) up.push((as * Math.pow(10, decimals) / 1e8) * bchPriceUSD); }
			if (up.length >= 2) { const mn = Math.min(...up); const mx = Math.max(...up); ars = mn > 0 ? ((mx - mn) / mn) * 100 : null; }
		}
		...
		const mctUSD = (mcapTvlThresholdSats / 1e8) * bchPriceUSD * 2;
		...
		return {
			...
			watchlistCount,
			moverBadges: { gainerRank: moverRanks.gainerRank, loserRank: moverRanks.loserRank, tvlMoverRank: moverRanks.tvlMoverRank, pricePct: moverE?.pricePct ?? null, tvlPct: moverT?.tvlPct ?? null },
			...
			cauldronTvlSharePct: ctv,
			arbitrage: { eligible: avp >= 2, venuesPresent: avp, rawSpreadPct: ars },
			herfindahlIndex: hi,
			topHolderSharePct: topHS, top10HolderSharePct: top10HS, firstNRank: firstNRankResult,
			bcmr: bcmr ? { status: bcmr.status, splitId: bcmr.splitId, uris: bcmr.uris, tags: bcmr.tags, extensions: bcmr.extensions, nftTypes: bcmr.nftTypes, nftsDescription: bcmr.nftsDescription } : null
		};
	});

	// ── Return: token shell immediately, everything else via streamed ──
	const token = {
		id: hexFromBytes(row.category)!,
		tokenType: row.token_type, genesisBlock: row.genesis_block,
		...
		liveUtxoCount: row.live_utxo_count, liveNftCount: row.live_nft_count,
		...
		firstNRank: null,  // populated inside streamed

...
<main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
	...
	<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
		<div class="p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40">
			<div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Total CRC-20</div>
			<div class="text-2xl font-bold text-slate-900 dark:text-white mt-1">{fmt(data.counts.totalCategories)}</div>
			<div class="text-xs text-slate-500 dark:text-slate-400 mt-1">categories detected</div>
		</div>
		<div class="p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40">
			<div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Canonical winners</div>
			<div class="text-2xl font-bold text-slate-900 dark:text-white mt-1">{fmt(data.counts.canonicalWinners)}</div>
			<div class="text-xs text-slate-500 dark:text-slate-400 mt-1">one per symbol bucket</div>
		</div>
		<div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
			<div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Distinct symbols</div>
			<div class="text-2xl font-bold text-slate-900 dark:text-white mt-1">{fmt(data.counts.distinctSymbols)}</div>
			<div class="text-xs text-slate-500 dark:text-slate-400 mt-1">unique symbol bytes</div>
		</div>
		<div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
			<div class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">Contested</div>
			<div class="text-2xl font-bold text-slate-900 dark:text-white mt-1">{fmt(data.counts.contestedSymbols)}</div>
			<div class="text-xs text-slate-500 dark:text-slate-400 mt-1">categories with rivals</div>
		</div>
	</div>
	...
	<div class="mb-8 flex flex-wrap gap-3">
		...
		<a
			...
			class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-semibold"
		>
	...
	{#if contested.length > 0}
		<section class="mb-10">
			<h2 class="text-xl font-bold text-slate-900 dark:text-white mb-3">
				Contested symbols
				<span class="text-sm font-normal text-slate-500 dark:text-slate-400">({contested.length})</span>
			</h2>
			<p class="text-sm text-slate-600 dark:text-slate-300 mb-4 max-w-3xl">
			...
			<div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
				<table class="w-full text-sm">
					<thead class="bg-slate-50 dark:bg-slate-800/50 text-left text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">
						<tr>
					...
					<tbody class="divide-y divide-slate-100 dark:divide-slate-800">
						{#each contested as bucket (bucket.symbol + bucket.symbolIsHex)}
							<tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30">
								...
								<td class="px-4 py-3 text-right font-mono text-slate-700 dark:text-slate-300">{bucket.contenderCount}</td>
								<td class="px-4 py-3">
	...
	<section>
		<h2 class="text-xl font-bold text-slate-900 dark:text-white mb-3">
			All symbol buckets
			<span class="text-sm font-normal text-slate-500 dark:text-slate-400">({data.symbols.length})</span>
		</h2>
		{#if data.symbols.length === 0}
			<div class="p-8 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 text-center text-slate-500 dark:text-slate-400">
					...
					<tbody class="divide-y divide-slate-100 dark:divide-slate-800">
						{#each uncontested as bucket (bucket.symbol + bucket.symbolIsHex)}
							<tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30">
								...
								<td class="px-4 py-3 text-right font-mono text-slate-700 dark:text-slate-300">1</td>
								<td class="px-4 py-3">
									{#if bucket.canonicalCategory}
										<a
											href={`/token/${bucket.canonicalCategory}`}

	...
	{#if data.voteLeaders.totalVotes > 0}
		<section class="mb-8">
			...
			<p class="text-xs text-slate-500 dark:text-slate-400 mb-4">
				Logged-in users vote ↑ or ↓ on tokens. Score = up − down. Controversial =
				LEAST(up, down) × (up + down) — favours tokens with both volume and a balanced split.
			</p>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				{#each [
					{ label: 'Most upvoted', sub: 'highest net score', sortQuery: '?sort=upvoted', tone: 'emerald', items: data.voteLeaders.mostUpvoted },
					{ label: 'Most downvoted', sub: 'lowest net score', sortQuery: '?sort=downvoted', tone: 'rose', items: data.voteLeaders.mostDownvoted },
					{ label: 'Most controversial', sub: 'big & evenly split', sortQuery: '?sort=controversial', tone: 'amber', items: data.voteLeaders.mostControversial }
				] as col (col.label)}
